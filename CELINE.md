...
## פקודה פעילה · 5.9.2026 · 00:45

סלין, זו הפקודה. היא דוחה כל דבר אחר ברשימה עד שהיא נגמרת.

**מתחילה ב-M1, עכשיו.** ענף `celine/m1-count`. זו המשימה
הראשונה בשבוע 1 והיא אינה תלויה בשום דבר פתוח. שלוש פקודות
הפתיחה:

    git fetch origin main
    git checkout -b celine/m1-count origin/main
    grep -n "שמונה" marketing/*.md

הפלט של השורה השלישית הוא המדידה שלך "לפני", והוא צריך להראות
21 שורות בארבעה קבצים. תיקנת — מריצה אותה שוב, וזה ה"אחרי".
מה שמותר לכתוב במקום נמצא ב-`marketing/facts.md` ורק בו.

**שתי הגדרות סיום, שתיהן חייבות להתקיים:**
1. `grep -n "שמונה" marketing/*.md` מחזיר רק שורות שבהן "שמונה"
   אינו מניין אפליקציות.
2. `node .claude/qa/all.js` — הבדיקות הסטטיות עוברות.

### שלוש הוראות אֲסוּרוֹת בפקודה הזאת

**אחת. אל תיגעי בענף `naming/theory-rename-ready`.** הוא לא שלך.
הוא מחזיק את הקומיט `8ed3429` שמוחק את שדה `u` מכרטיס "תאוריה
מדברת", והוא חסום בכוונה: מדדתי היום ש-`/theory/` מחזיר 404
ושהריפו `lagstein1-png/theory` אינו קיים. הקומיט הזה, ברגע
שיגיע ל-`main`, שובר קישור חי למשתמש. הוא ייכנס אחרי ששם הריפו
הנפרד ישונה, וזה לא בידיים שלך ולא בידיים שלי.

**שתיים. אל תמחקי את השם האסור מ-`index.html` שוב.** שדה `u`
נשאר כמו שהוא. `naming.js` מחריג אותו במפורש והבדיקה עוברת —
"100 קבצים, 0 ממצאים". אם ראית אותו וחשבת שזה פער, זה לא פער,
זו החלטה.

**שלוש. אין שם חלופי, ולא ממציאים אחד.** ראיתי `EasyTheory`
במסמכים שמחוץ לריפו. בדקתי: `easytheory` ו-`EasyTheory` שניהם
404 בגיטהאב. השם הרשמי היחיד הוא **"תאוריה מדברת"**, לפי שורת
הטבלה ב-`NAMING.md`. מסמכים שמחוץ לריפו הזה אינם בתחום שלך.

**הדוח:** בסוף העבודה, בתבנית שבסוף הקובץ הזה. מספר בלי מקור
לא נכנס לדוח.
...
### קו הבסיס שמדדתי לך, 5.9.2026 00:50

שלוש־עשרה כתובות, כולן 200. זה המצב שאת מאמתת שלא נשבר:

| נתיב | תשובה | גודל |
|---|---|---|
| `/` | 200 | 62,085 |
| `/math-app/` | 200 | 327,748 |
| `/math-teen/` | 200 | 618,097 |
| `/math-uni/` | 200 | 1,000,552 |
| `/math-uni2/` | 200 | 899,714 |
| `/math-uni3/` | 200 | 783,808 |
| `/bagrut-806/` | 200 | 22,274 |
| `/ulpan/` | 200 | 367,523 |
| `/english/` | 200 | 443,400 |
| `/history/` | 200 | 1,084,847 |
| `/lomda/` | 200 | 260,946 |
| `/reader/` | 200 | 77,291 |
| הנתיב של "תאוריה מדברת" | 200 | 198,447 |

הפקודה שמייצרת את הטבלה מחדש, וזו המדידה שלך ולא העתקה שלי:

    for p in "" math-app math-teen math-uni math-uni2 math-uni3 \
...
הכתובת שלו לא נכתבת בקובץ הזה.

### חלק א׳ · מה שנמדד בפקודה

1. שלוש־עשרה כתובות, כולן 200. שורה שאינה 200 — **קריטי**.
2. לכל אפליקציה `manifest.json` ו-`sw.js` מחזירים 200.
3. הגודל של כל דף אינו קטן משמעותית מהטבלה למעלה. דף שהצטמק
   בחצי הוא דף שנחתך.

### חלק ב׳ · מה שנבדק ביד, ואין לו פקודה

**בטלפון ובמחשב. שניהם. כל אפליקציה.** לא מדגם.

לכל אחת מ-12 האפליקציות, בסדר שבדף הבית:

1. הכרטיס בדף הבית נפתח לאפליקציה הנכונה. הכותרת שבפנים זהה
   לשם שבכרטיס — זה הפער מ-`NAMING.md`, בין מקום 4 למקום 5.
2. שאלה אחת נטענת, ויש **ארבע** אפשרויות. פחות מארבע — **קריטי**,
   ורושמים באיזו רמה ובאיזה נושא.
3. תשובה נכונה ותשובה שגויה: שתיהן מקבלות תגובה, וההסבר מופיע.
4. הרמז נפתח ואומר משהו שקשור לשאלה.
5. **ההקראה מדברת בקול.** שותקת, או מבטאת מילה עברית לא נכון —
   שורה בדוח עם המכשיר, הדפדפן והמילה. זה התחום של `renana`.
6. החלפת שפה: ארבע השפות, ואין מחרוזת שנשארה בעברית באמצע מסך
   בערבית או ברוסית. בערבית — הכיווניות מימין לשמאל לא נשברת.
7. רענון הדף באמצע: המקום נשמר, לא מתאפס.
8. הכפתורים גדולים מספיק לאצבע בטלפון, ואין גלילה אופקית.
9. הקונסולה (F12) בלי שגיאה אדומה. יש — מעתיקים אותה כלשונה.

ובדף הבית עצמו: 12 כרטיסים, החיפוש מוצא אפליקציה לפי מילה
מהתיאור, והסינון לפי קטגוריה מחזיר את מה שהוא מבטיח.

### הדוח על הבדיקה הזאת

טבלה, שורה לכל אפליקציה, וארבע עמודות: אפליקציה · טלפון · מחשב ·
מה נשבר. בלי ממצא — כותבים "תקין" ולא משאירים ריק. וכן:

    מכשיר: <דגם> · <דפדפן ומספר גרסה>
    מחשב:  <מערכת> · <דפדפן ומספר גרסה>

בלי שתי השורות האלה הדוח אינו ניתן לשחזור, ולכן אינו דוח.

**ממצא קריטי אינו מחכה לסוף הבדיקה.** מצאת מסך לבן, 404 או
הקראה שותקת — שולחת לי מיד, וממשיכה לבדוק את השאר.
