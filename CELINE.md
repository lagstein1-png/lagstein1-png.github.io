...
"103 קבצים, 0 ממצאים" (5.9). אם ראית אותו וחשבת שזה פער, זה לא
פער, זו החלטה — והיא כבר נשברה פעם אחת ב-`main` ותוקנה בו.
...
## פקודה 3 · אחרי בדיקת הקבלה · מה שעלה מהמפה

הגיעה מפת דרכים של 21 מועמדות לאפליקציה. הפסק ההנדסי המלא הוא
`ROADMAP.md` בשורש — **קראי אותו לפני שאת נוגעת במשהו מהפקודה
הזאת.** שלושה דברים ממנו הם שלך.

### 3א · חמישה קובצי תוכן, לא חמש אפליקציות

חמש מהמועמדות אינן אפליקציה אלא קובץ ב-`lomda/data/`. הנוהל
בשלב 0.5 של `.claude/skills/create-learning-app/SKILL.md` הוא
הפוסק, ושלוש הנגיעות כתובות שם: קובץ שקורא ל-`BANK(...)`, תגית
`<script src="data/<שם>.js">` ב-`lomda/index.html` **לפני הסקריפט
של המנוע**, ואותו נתיב ב-`PRE` שב-`lomda/sw.js`.

הסדר, וענף לכל אחד:

| סדר | קובץ | הנושא | ענף |
|---|---|---|---|
| 1 | `lomda/data/rights.js` | מיצוי זכויות מול המוסדות | `celine/lomda-rights` |
| 2 | `lomda/data/work.js` | קורות חיים וראיון עבודה | `celine/lomda-work` |
| 3 | `lomda/data/safety.js` | בטיחות בעבודה | `celine/lomda-safety` |
| 4 | `lomda/data/geo.js` | גיאוגרפיה לחטיבה | `celine/lomda-geo` |
| 5 | נושא ב-`civics.js` הקיים | מבחן התאזרחות | `celine/lomda-citizen` |

**קובץ אחד בכל ענף.** הסכימה היא `lomda/data/schema.js` ולא
המצאה. לפני ה-commit: `node .claude/qa/banks.js` — הוא בודק את
שלוש הנגיעות, והמספרים בדוח שלך מגיעים מהפלט שלו. ב-5.9 הוא
מחזיר "5 מאגרים · 9 נושאים · 117 פריטים · 6 קובצי data" — הנושא
החמישי, בריאות ותזונה, נכנס באותו יום.

**התגית אחרי המנוע היא באג שקט** — הנוהל מזהיר שהנושאים יוצאים
ריקים ושום שגיאה לא נזרקת, ו-`smoke.js` החזיר על זה `clean`
בעבר. לכן פותחים את לומדה בדפדפן ורואים את הנושא בעיניים.

### 3ב · שלוש שאלות שם — טבלה, לא שינוי

ארבעה שמות במפה אינם עומדים ב-`NAMING.md`, וזה כתוב ב-`ROADMAP.md`
תחת "שמות שחייבים להשתנות". שלושה מהם צריכים הצעה חלופית:
**מפתן**, **תא**, **אות**. לכל אחד: שתי הצעות, ולכל הצעה התעתיק
בארבע שפות לפי הכלל, ובדיקה שהוא אינו מתנגש בשתים־עשרה הקיימות
ולא ב"מפנה". **טבלה בדוח. אני מחליט, ואז את מבצעת.**

### 3ג · שני דברים שאסור, ושניהם נמדדו

**האחד — הענף `claude/new-exam-app-xkohvq` אינו נקודת פתיחה.**
מדדתי: 105 קומיטים ב-`main` אינם בו, ו-4 קומיטים בו אינם ב-`main`.
מיזוג שלו יחזיר את הריפו חודש לאחור. לא לבנות עליו כלום.

**השני — יש עכשיו שער מכני על הכרטיס של "תאוריה מדברת".**
מחיקת שדה `u` בוצעה כאן פעמיים, ובשתיהן היא היתה שוברת קישור
חי ל-404. לכן `node .claude/qa/apps.js` נכשל מהיום אם השדה חסר:

    ✗ theory: אין נתיב מפורש בכרטיס. המזהה לבדו מוביל ל-404

ראית את השורה הזאת — **הפתרון אינו למחוק את הבדיקה.** הפתרון
הוא להחזיר את השדה. הבדיקה נמחקת יחד עם השדה, ורק אחרי ששם
הריפו הנפרד ישונה. הנימוק ב-`NAMING.md`.

### מה לא נבנה, וזו תשובה לשאלה שנשאלה

לאף אחת מ-21 המועמדות אין התחלה במאגר: אין תיקייה, אין ענף, ואין
קובץ. `lomda/data/` מכיל ב-5.9 את `civics.js`, `digital.js`,
`health.js`, `money.js`, `science.js` ו-`schema.js` — וזה הכול
(`ls lomda/data`). מי שיאמר לך ש"התחילו את זה" — תבקשי את שם
הקובץ.

---

## פקודה 4 · ארבע שפות בכל אפליקציה — שלוש חסרות, בסדר הזה

**ההנחיה מהמנהל, 5.9.2026:** לא כל האפליקציות בארבע שפות, וזה
צריך להיפתר. זו הפקודה. היא נכנסת אחרי פקודה 2 (בדיקת הקבלה),
ולפני פקודה 3.

### מה נמדד, 5.9.2026

`var LANGS` בראש הסקריפט של כל אפליקציה, ותגית `tLangs` בכרטיס
שבדף הבית (`DATA.APPS`, שדה `t`):

| אפליקציה | `LANGS` | תג בדף הבית | מנגנון תרגום |
|---|---|---|---|
| תשע אפליקציות | he ar ru en | `tLangs` | קיים ומלא |
| `reader` | he ar | חסר | `TR.ar` — 102 מפתחות |
| `math-teen` | he ar | חסר | `TR_AR` — 1,848 מפתחות, ועוד `TR_AR_XQ` — 10 |
| `bagrut-806` | **אין `LANGS` כלל** | חסר | **אין מנגנון.** `lang="he"` בלבד, ו-163 מחרוזות עבריות בתוך `app.js` |

המקורות: `grep -n '^var LANGS={'` בכל `index.html`; `node` שספר
את מפתחות המילונים; `grep -c` על `bagrut-806/app.js`.
`marketing/facts.md` שורות 15–19 אומר את זה במילים: "בתשע מתוך
שתים־עשרה".

**הסדר הוא לפי גודל, מהקטן לגדול. ענף לכל אפליקציה, ולא ענף
אחד לשלושתן** — מילון תרגום הוא שורה אחת ענקית וכל מיזוג שלו
מתנגש במלואו (`CLAUDE.md`, "מיזוג של מילון תרגום הוא איחוד").

### 4א · `reader` — 102 מפתחות · ענף `celine/reader-4lang`

1. ב-`reader/index.html` שורה 560: `var LANGS` — להוסיף `ru` ו-`en`
   בדיוק בצורה שב-`math-uni/index.html` שורות 879–884, כולל `dir:"ltr"`.
2. שורה 564: `var TR` — להוסיף `"ru":{…}` ו-`"en":{…}` עם **אותם
   102 מפתחות** של `"ar"`. המפתח הוא המחרוזת העברית, אות באות.
3. ההערה מעל `LANGS` אומרת: "שתי השפות מימין לשמאל, ולכן אין כאן
   היפוך פריסה". עם רוסית ואנגלית זה כבר לא נכון — לבדוק בדפדפן
   שהפריסה מתהפכת ב-`dir:"ltr"`, ולתקן את ההערה.
4. `TTSMSG` בשורה 932 כבר בארבע שפות — לא נוגעים.

**סיום:** בדפדפן, ברוסית ובאנגלית, אין מילה עברית אחת בממשק
(הטקסט שהמשתמש מדביק הוא עברית — זה בסדר, זו האפליקציה).
`node .claude/qa/arabic.js reader` עובר. `node .claude/qa/all.js` עובר.

### 4ב · `math-teen` — 1,858 מפתחות · ענף `celine/teen-4lang`

1. שורה 687: `var LANGS` — להוסיף `ru` ו-`en` כמו ב-`math-uni`.
2. שורה 730: `var TR={ar:(typeof TR_AR!=="undefined"?TR_AR:{})}` —
   להוסיף `ru:` ו-`en:` באותה צורה, עם `TR_RU` ו-`TR_EN`.
3. שני מילונים חדשים, **בבלוק נפרד לפני שורה 422** (איפה ש-`TR_AR`
   יושב, ומאותה סיבה שכתובה שם: מילון שנטען אחרי הציור הראשון
   מאחר במסך שלם): `TR_RU` ו-`TR_EN`, כל אחד עם **אותם 1,848 מפתחות**
   של `TR_AR`. הפקודה שמפיקה את רשימת המפתחות:

        node -e 'const s=require("fs").readFileSync("math-teen/index.html","utf8");const l=s.slice(s.indexOf("var TR_AR="));const o=eval("("+l.slice(10,l.indexOf("\n")).replace(/;$/,"")+")");console.log(JSON.stringify(Object.keys(o),null,0))' > /tmp/teen-keys.json

   ואותו דבר ל-`TR_AR_XQ` (10 מפתחות), אל `TR_RU_XQ` ו-`TR_EN_XQ`.
4. **לא מתרגמים ביד 1,848 מחרוזות בבת אחת.** עובדים נושא־נושא:
   פותחים את "שלב" ברוסית, עוברים נושא, מריצים `trTodo()` בקונסולה,
   ומתרגמים את מה שחזר. 18 נושאים, 18 סבבים, commit אחרי כל סבב.
   כך גם באנגלית. הסוכנת `maayan` היא הכתובת לניסוח.
5. הפריסה: `math-teen` מכיל היום 6 אזכורים של `ltr` — לבדוק בדפדפן
   שמסך שאלה, דף הנוסחאות ובונה המבחן מתהפכים נכון.

**סיום:** `trTodo()` מחזיר רשימה ריקה אחרי סיור מלא, ברוסית
ובאנגלית. `node .claude/qa/all.js` עובר. בדוח: כמה מפתחות תורגמו
בכל סבב, מהפלט.

### 4ג · `bagrut-806` — אין מנגנון, ולכן קודם מנגנון · ענף `celine/806-4lang`

זו לא משימת תרגום. זו משימת הנדסה, ורק אחריה תרגום.

1. **המנגנון** מועתק מ-`math-uni/index.html` שורות 879–891:
   `LANGS`, `LG`, `TR={}` והפונקציה `_()`. ב-806 הקוד יושב
   ב-`app.js`, ושם הוא נכנס — לפני כל מחרוזת שמוצגת.
2. **163 המחרוזות** ב-`app.js`: כל אחת נעטפת ב-`_("…")`. הפקודה
   שמונה אותן לפני ואחרי:

        grep -o '"[^"]*[א-ת][^"]*"' bagrut-806/app.js | wc -l
        grep -o '_("[^"]*[א-ת][^"]*")' bagrut-806/app.js | wc -l

   השורה השנייה צריכה להגיע למספר של הראשונה.
3. בורר שפה בממשק, כמו ב-`math-uni` — לחפש שם `Object.keys(LANGS)`.
4. רק אז שלושה מילונים, ורק לממשק. **תוכן הבחינות ב-`data/exams.js`
   נשאר בעברית בשלב הזה** — זו בחינה בעברית, והתרגום שלה הוא
   החלטה נפרדת שאני מעלה למנהל אחרי שהממשק עובד.

**סיום:** `var LANGS` ב-`app.js` עם ארבע שפות; הממשק ברוסית
ובאנגלית בלי עברית; `node .claude/qa/exam806.js` ו-`all.js` עוברים.

### אחרי כל אחת משלוש

- **תג בדף הבית:** ב-`index.html` של השורש, ברשומת האפליקציה
  ב-`DATA.APPS`, להוסיף `"tLangs"` למערך `t`. זה שינוי בדף הבית
  ולכן "דורש העלאת גרסה" בדוח — של האפליקציה ושל דף הבית.
- **`marketing/facts.md` שורות 15–19 ו-51–54:** לעדכן את המניין
  לפי מה שנמדד אחרי המיזוג, לא לפני. "תשע מתוך שתים־עשרה" הופך
  ל"עשר", "אחת־עשרה", ולבסוף "כולן" — רק כשזה נכון.
- `node .claude/qa/status.js --md` לדוח.

### מה לא עושים כאן

- לא מתרגמים דרך שירות בתשלום או API. תרגום ביד, או בעזרת
  הסוכנת `maayan`.
- לא משנים מחרוזת עברית כדי "להקל על התרגום" — המחרוזת העברית
  היא המפתח, ושינוי שלה מוחק את הערבית הקיימת בשקט.
- לא נוגעים ב-`TR_AR` הקיים חוץ מלהעתיק ממנו מפתחות.
...
### D1 · סגור · שתי רמות באקסיומה מציגות שלוש אפשרויות — בכוונה

**עדכון 5.9.2026 (סופר):** זה אינו באג אלא קו בסיס מתועד.
`.claude/qa/README.md`, פרק "שורות `under4` ב-math-uni שנבדקו
ונמצאו תקינות": ב-`lineq L2` השאלה היא "כמה פתרונות יש למערכת" —
פתרון יחיד / אין / אינסוף — וב-`logic L1` "מהו ערך האמת" — אמת /
שקר / תלוי. אלה כל התשובות האפשריות, ואפשרות רביעית הייתה שקר.
המשימה נשארת כאן כתיעוד של מה נבדק; **אין מה לעשות בה.** הניסוח
המקורי:
...
**בוצע 5.9 בענף `claude/global-social-growth-strategist-j2neaf`, לא ממוזג**, יחד עם
