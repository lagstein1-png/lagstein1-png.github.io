...
/* גובה הקול כשאין נשי במכשיר. **זה אינו קול נשי** אלא אותו קול
   ...
   **1.45, ואחריו שתי טעויות שלי — שתיהן נמדדו ולא שוערו.**

   הראשונה: 1.12. זה ה-`pitch` של הפריסט `calm-female`, כלומר
   הגובה של קול נשי **אמיתי** — ולא ההרמה כשאין כזה. ההפרש בינו
   לבין הרמה של ממש הוא בדיוק ״לא שמעתי שינוי״.

   השנייה: 1.35. זה אמנם `FEM_PITCH` שבאפליקציות, אבל הן אינן
   מדברות בו: הן מכפילות אותו בגובה הפריסט וחוסמות בתקרה —
   `Math.min(1.45, pitch * femLift(...))` — ולכן על מכשיר בלי
   קול נשי הן משמיעות **1.45**. לג׳וש אין פריסט, ולכן 1 × 1.35
   נותן 1.35, והוא נשמע נמוך מהאפליקציה שהוא יושב בתוכה.

   נמדד במנוע דיבור מזויף, 13.9.2026: האפליקציה אמרה ״שועל״
   ב-Asaf בגובה 1.45, וג׳וש אמר את תשובתו באותו קול בגובה 1.35.
   שני קולות באותו מסך, ואחד מהם נמוך יותר.

   לכן הערך כאן הוא **התקרה שהאפליקציות מגיעות אליה**, ולא
   הקבוע שהן מכפילות. `node .claude/qa/tutor.js` משווה אותו
   ל-`Math.min` שבקוד שלהן, כדי שהשניים לא ייפרדו שוב. */
var FEM_PITCH = 1.45;
...
function speakSeg(text, code, r, alive, done){
  ...
  (function go(){
    ...
    /* השמה של קול שהמנוע דוחה זורקת, והזריקה הזאת מפילה את כל
       מסלול ההקראה — הלומד מקבל שקט בלי הודעה. נתפס 13.9.2026
       בבדיקה עם מנוע מזויף. הקול הוא העדפה ולא תנאי: בלעדיו
       הדפדפן מדבר בברירת המחדל שלו, וזה עדיף על שקט. */
    var v = pickVoice(code);
    if(v){ try{ u.voice = v }catch(e){ v = null } }
