...
/* קצב מעט איטי מסייע לקריאה. גובה ניטרלי שומר על הקול המקורי של המכשיר. */
var JOSH_PITCH = 1;
var JOSH_RATE  = 0.95;
...
he:{ btn:"לימור — עזרה מהמוֹרָה", title:"עזרה מהמוֹרָה", close:"סגירה", send:"שליחה",
  ...
  edgeNote:"הקול במחשב הזה בסיסי ונשמע רובוטי. בדפדפן Microsoft Edge, שמותקן בכל Windows, לימור מדברת בקול טבעי — פותחים את אותו הדף שם.",
  manNote:"לא זוהה כאן קול נשי בשפה הזאת. אם נשמע קול גברי, אפשר להתקין קול נשי במכשיר או לבחור אותו מרשימת הקולות.",
  simplifyBtn:"לימור — גרסה פשוטה", simplifyAsk:"תן לי גרסה קצרה ופשוטה של הטקסט שהדבקתי",
  simplifyEmpty:"קודם הדביקו טקסט, ואז אבקש מלימור גרסה פשוטה שלו.",
...
ar:{ btn:"ليمور — مساعدة من المعلّمة", title:"مساعدة من المعلّمة", close:"إغلاق", send:"إرسال",
  ...
  edgeNote:"الصوت على هذا الحاسوب بسيط ويبدو آليًّا. في متصفّح Microsoft Edge، المثبّت في كل Windows، تتكلّم ليمور بصوت طبيعي — افتحوا الصفحة نفسها هناك.",
  manNote:"لم يُعثر على صوت نسائيّ لهذه اللغة. إذا بدا الصوت رجاليًّا، ثبّت صوتًا نسائيًّا على الجهاز أو اختره من قائمة الأصوات.",
  simplifyBtn:"ليمور — نسخة مبسّطة", simplifyAsk:"أعطني نسخة قصيرة وبسيطة من النصّ الذي لصقته",
  simplifyEmpty:"الصقوا نصًّا أولًا، ثم أطلب من ليمور نسخة مبسّطة منه.",
...
ru:{ btn:"Лимор — помощь учительницы", title:"Помощь учительницы", close:"Закрыть", send:"Отправить",
  ...
  edgeNote:"Голос на этом компьютере простой и звучит как робот. В браузере Microsoft Edge, который есть в каждом Windows, Лимор говорит естественным голосом — откройте эту же страницу там.",
  manNote:"Женский голос для этого языка не найден. Если голос звучит мужским, установите женский голос на устройстве или выберите его в списке.",
  simplifyBtn:"Лимор — простая версия", simplifyAsk:"Дай мне короткую и простую версию вставленного текста",
  simplifyEmpty:"Сначала вставьте текст, и тогда я попрошу у Лимор его простую версию.",
...
en:{ btn:"Limor — ask the teacher", title:"Ask the teacher", close:"Close", send:"Send",
  ...
  edgeNote:"The voice on this computer is basic and sounds robotic. In Microsoft Edge, which comes with every Windows, Limor speaks in a natural voice — open this same page there.",
  manNote:"No female voice was identified for this language. If the voice sounds male, install a female voice on your device or choose one from the voice list.",
  simplifyBtn:"Limor — simple version", simplifyAsk:"Give me a short, simple version of the text I pasted",
  simplifyEmpty:"Paste a text first, and then I will ask Limor for a simple version of it.",
...
/* ההסבר ״אפשר לשאול אותי...״ הוא אוריינטציה חד־פעמית, לא כותרת קבועה.
   ...
   הזה הוא היה חוזר בכל פעם — נראה כאילו לימור מציג את עצמו כל פעם
...
/* ---------- בחירת קול: קול פעיל קודם, ואז קול נשי, טבעיות ודיוק שפה.
   שם יצרן אינו הוכחה למגדר. בחירה ידנית שמורה קודמת למיון. ---------- */
...
/* 2 = קול נשי, 0 = קול גברי, 1 = מגדר לא ידוע. שם הפונקציה נשמר
   לשם תאימות עם בדיקות קיימות; דירוג ההעדפה תמיד קודם לאיכות. */
function femScore(v){
...
/* הבחירה הידנית של הלומד נשמרת לכל שפה בנפרד. */
...
/* **סימני LaTeX — 23.9.2026.** המודל כותב לפעמים ״$(5, 0)$״ למרות
   ...
   כולו — לימור בכל שלוש־עשרה — לא היה נטען. */
function noTex(text){
...
function speakSeg(text, code, r, alive, done){
  ...
  (function go(){
    ...
    /* אין שינוי מלאכותי בגובה הקול, גם כשאין קול נשי מזוהה.
       באנדרואיד מנוע הדיבור עשוי להתעלם ממילא מ-pitch. */
    u.pitch = JOSH_PITCH;
...
/* ---------- מנוע ההגייה העברי, 18.9.2026 ----------
   ...
   **לימור היה הנתיב היחיד באתר שלא עבר בו.** `/tutor/he-speech.js`
   ...
   כלומר הבעלים התקין מנוע הגייה, ולימור — שהוא הקול שמדבר אליו
   ישירות — המשיך לומר ״משלש״, ״מצין״ ו״מאד״.

   **ומתוך 169 המחרוזות הכתובות של לימור רק ארבע נשמעות אחרת**
   ...
   **מוגן, ועברית בלבד** — אותה תבנית בדיוק כמו ב-`bagrut-806`:
   ...
   לא להיות שם. תלות קשיחה הייתה משתיקה את לימור לגמרי. */
...
/* ================= הפאנל ================= */
var CSS = ''
/* ---- מנוע לימור, 16.9.2026: הפאנל אינו מסתיר את התרגיל ----

   עד כאן הפאנל היה חלון במרכז המסך מעל רקע מוחשך — כלומר לימור
   כיסה את השאלה, את האפשרויות ואת הכפתורים בדיוק ברגע שהלומד
   ביקש עזרה עליהם, ופעולה כמו ״הדגש אפשרות״ הייתה קורית מאחורי
   וילון. המנדט: ״לימור הוא עזר ולא המטרה. הוא לא מסתיר שאלה,
   תשובות או כפתורים״.
...
/* כפתורים גדולים וצבעוניים — גובה 46px כמו שאר כפתורי המגע
   באפליקציות. */
+'.tu-ctl button{border:2px solid #0b7568;background:#0e9c8d;color:#fff;font-weight:700;'
+'border-radius:11px;padding:9px 17px;font:inherit;font-size:1.02rem;cursor:pointer;min-height:46px}'
+'.tu-ctl button[data-tu="stop"]{background:#bf4426;border-color:#93331b}'
+'.tu-ctl button[aria-pressed="true"]{background:#0b7568;border-color:#084f45}'
+'.tu-ctl .tu-sys{font-size:1rem}'
+'.tu-ctl select{border:2px solid #0b7568;border-radius:11px;padding:8px 10px;'
+'font:inherit;font-size:1.02rem;min-height:46px;background:#fff;color:#17333c;font-weight:600}'
...
+'#tu-in,.tu-ctl select,#tu-lg{background:#1e2f38;color:#eef5f7;'
+'border-color:rgba(238,245,247,.3)}'
+'.tu-ctl button{background:#0e9c8d;color:#fff;border-color:#0b7568}'
+'.tu-ctl button[data-tu="stop"]{background:#bf4426;border-color:#93331b}'
...
/* לאן גוללים אחרי ציור. עד 18.9.2026 — תמיד לתחתית, וזה הסתיר את
   ...
   האחרונה היא של לימור — ראש הבועה שלו בראש אזור השיחה, והלומד
   קורא מההתחלה וגולל למטה אל ההמשך, ההקראה וההצעות. כשהילד
   שלח, כשלימור עדיין חושב, או כשיש הערה — התחתית, כמו קודם.
...
/* **איך משיגים קול נשי — 17.9.2026.**
   `manNote` מסבירה מה לעשות כשאין קול נשי מזוהה ולא משנה pitch.
   מסלולי ההגדרות תלויים במערכת ההפעלה. */
function voiceHow(){
  var ua = navigator.userAgent || "", L = lang();
  var A = { he:"באנדרואיד: הגדרות ← נגישות ← טקסט לדיבור (בסמסונג: הגדרות ← ניהול כללי ← טקסט לדיבור) ← גלגל השיניים ליד המנוע ← התקנת נתוני קול ← לבחור וריאנט נשי.",
            ar:"في أندرويد: الإعدادات ← إمكانية الوصول ← النص إلى كلام (في سامسونج: الإعدادات ← الإدارة العامة ← النص إلى كلام) ← الترس بجانب المحرّك ← تثبيت بيانات الصوت ← اختيار نسخة نسائيّة.",
            ru:"На Android: Настройки ← Специальные возможности ← Синтез речи (в Samsung: Настройки ← Общие настройки ← Синтез речи) ← шестерёнка рядом с движком ← Установка голосовых данных ← выбрать женский вариант.",
            en:"On Android: Settings → Accessibility → Text-to-speech (on Samsung: Settings → General management → Text-to-speech) → the gear next to the engine → Install voice data → pick a female variant." };
  var I = { he:"באייפון או באייפד: הגדרות ← נגישות ← תוכן מדובר ← קולות ← לבחור קול נשי ולהוריד.",
            ar:"في آيفون أو آيباد: الإعدادات ← إمكانية الوصول ← المحتوى المنطوق ← الأصوات ← اختيار صوت نسائيّ وتنزيله.",
            ru:"На iPhone или iPad: Настройки ← Универсальный доступ ← Устный контент ← Голоса ← выбрать женский голос и загрузить.",
            en:"On iPhone or iPad: Settings → Accessibility → Spoken Content → Voices → pick a female voice and download it." };
  var M = { he:"במק: הגדרות המערכת ← נגישות ← תוכן מדובר ← קול המערכת ← ניהול קולות ← להוריד קול נשי בעברית.",
            ar:"في ماك: إعدادات النظام ← إمكانية الوصول ← المحتوى المنطوق ← صوت النظام ← إدارة الأصوات ← تنزيل صوت نسائيّ.",
            ru:"На Mac: Системные настройки ← Универсальный доступ ← Устный контент ← Системный голос ← Управление голосами ← загрузить женский голос.",
            en:"On Mac: System Settings → Accessibility → Spoken Content → System Voice → Manage Voices → download a female voice." };
...
function sendLegacy(text, auto){
  ...
  /* O-72 — אותו timeout כמו fetchWithTimeout ב-barak-core.js:227:
     ...
     בלי זה ספק שתולה השאיר את הפאנל ב-״רגע, חושבת…״ לנצח — נמדד
