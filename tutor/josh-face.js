...
/* **ברירת המחדל היא האדם המצויר, לא הרובוט — 15.9.2026.**

   כשאין תצלום, הנפילה לאחור חייבת להיות פרצוף אנושי: ראש כרום
   שמופיע פתאום במקום דיוקן הוא לא ״גיבוי״, הוא דמות אחרת.
   נמדד באותו יום ב-`math-app` — `JF_PHOTO` לא הוחל, והפנים
   הגדולות היו הרובוט המצויר. */
var look = "human";
...
/* ---------------------------------------------------------------
   ...
   --------------------------------------------------------------- */
var CSS = [
/* **ציפה — כל הפנים, מ-15.9.2026.**

   הבעלים: ״תעבור על הפנים, כולם לא זז״. נמדד ב-12 שניות בדפדפן:
   **שני מצמוצים, וזהו** — ואין שום אנימציה רציפה על `.jf`.
   מצמוץ נמשך 120 מילישניות, ולכן הפנים סטטיות כ-98% מהזמן.
   זה נכון גם לפני התיקון וגם אחרי: מה שחסר לא היה המצמוץ אלא
   **תנועה שרואים בין המצמוצים**.

   **שישה פיקסלים ב-3.4 שניות, ולא יותר.** תנועה שמושכת את העין
   מהטקסט היא ההפך ממה שדרוש ללומד עם קשיי קשב — זה אותו שיקול
   שקבע את הנשימה ל-1.5%.

   `jf-nod` שב-״נכון״ יושב על אותו צומת ודורס אותה לרגע, וזה
   מכוון: הנהון הוא רגע, והציפה חוזרת מעצמה. `prefers-reduced-motion`
   מכבה את שתיהן בבלוק שבסוף הקובץ. */
".jf{display:inline-block;line-height:0;position:relative;" +
  "animation:jf-float 3.4s ease-in-out infinite}",
"@keyframes jf-float{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}",
...
/* ---------------------------------------------------------------
   ...
   לימור המצוירת: שיער עד הכתפיים, פנים עדינות, צוואר וחולצה. הגוונים באים
...
function faceSVG(px) {
  return '<svg viewBox="0 0 120 120" width="' + px + '" height="' + px + '" aria-hidden="true" focusable="false">' +
    ...
    '<g class="jf__head">' +
      /* ---- שיער אחורי עד הכתפיים, ואוזניים ---- */
      '<path fill="' + HAIR + '" d="M 29 100 Q 34 84 30 58 Q 28 24 46 15 Q 61 8 76 17 Q 92 28 89 59 Q 86 81 92 100 Q 81 93 76 80 L 43 80 Q 38 95 29 100 Z"/>' +
      ...
      '<path fill="#e1a090" opacity=".24" d="M 41 68 Q 48 75 60 75 Q 72 75 79 68 Q 77 82 60 88 Q 43 82 41 68 Z"/>' +

      /* ---- שיער קדמי: קו שיער סרוק לצד, ורקות ---- */
      '<path fill="' + HAIR + '" d="M 32 54 Q 27 20 47 15 Q 72 4 87 29 Q 91 44 86 56 Q 83 36 75 32 Q 64 43 39 40 Q 36 45 32 54 Z"/>' +
...
var JOSHFACE = {
  ...
  /* **הכיול של `img/paula.jpg`, נמדד ולא נוחש.**

     הבעלים שלח 14.9.2026 תמונה שיצר — שתי דמויות מול עץ — וכתב
     ״תמחק את הגבר״. הדמות השמאלית נחתכה ל-200×300, אותו יחס
     שהקובץ הקודם נשא, ואחרי ארבעה חיתוכי מועמד שנצפו זה לצד זה
     נבחר זה שאין בו שריד של הדמות השנייה — שלושה מהארבעה נשאו
     שבב אפור בפינה הימנית־עליונה.

     הנקודות נקראו מרשת אחוזים שהונחה על החתך עצמו, בדיוק כפי
     ש-`MARKS_BOT` נמדד:

         עין ימין (של הצופה)   64%  , 53%
         עין שמאל              92%  , 54.5%
         פה                    80%  , 80%     · בסיס האף 73%

     **הפנים כאן פונות הצידה**, ולכן העין השנייה קרובה לשפה
     ורוחב המסכה שלה קטן במעט. זה מכוון: מסכה רחבה שם הייתה
     גולשת אל מחוץ לפנים. */
  MARKS_PAULA: {
    ax: "64%", ay: "53%",   ew: "8%",  eh: "3%",
    bx: "92%", by: "54.5%", ew2: "7%", eh2: "3%",
    jx: "80%", jy: "80%",   jw: "13%", jh: "5%", jo: "73%"
  },
