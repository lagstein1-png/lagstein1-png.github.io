...
### ריצה 5 — הכשל שהמנוע נבנה לסגור, דרך תשובה מוצלחת

**אישרה את תיקון השם:** כל חמשת אזכורי השם ברוסית היו `Барак`
בקירילית. אפס לטיני, אפס עברי.

**ומצאה את זה:**

    ✗ english ru next   ״נעבור״ בלי פעולה next_question

המודל הכריז על מעבר ולא החזיר קריאת פונקציה, והשרת החזיר **200**.
זה הכשל השני שבמנדט, מהכיוון שלא נשמר עליו: הנפילה המקומית מכסה
רשת, timeout, 429 ו-5xx — **תשובה מוצלחת בלי פעולה לא נבדקה כלל.**
המוטציה בדפדפן הראתה את זה בשמונה אפליקציות, לא באחת.

`claimed()` דורש **שני** תנאים — הלומד ביקש במפורש, **וגם** המודל
הכריז. תנאי אחד לבדו נמדד וגרוע מהתקלה: ״מה הצעד הבא בפתרון?״
היה מדלג לשאלה הבאה באמצע הסבר.

**הכלל המצטבר מארבע הריצות של הערב, והוא לסשן הבא:** שלוש תקלות
נמצאו בקריאת הפלט ולא בפסק — אותיות עבריות בערבית, שתי צורות של
השם ברוסית, והכרזה בלי ביצוע. **פסק ירוק אומר ״לא נפל על מה
שנבדק״, ולא ״תקין״.** מודל אמיתי מייצר מחרוזות שאיש לא חשב
עליהן, ולכן קוראים אותן.
