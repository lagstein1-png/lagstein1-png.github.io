...
### ריצה 4 — O-62 נסגר, והפלט הצריך סבב נוסף

`english` ב-`ar,ru`, 8 תרחישים, 16 קריאות, לא נעצרה בתקרה, אפס כשלים.

**נסגר:** אפס אותיות עבריות בערבית וברוסית, והערבית התייצבה על
כתיב אחד (`باراك` בשלושת האזכורים, ולא `بارق` כמו בריצה 3).

**ונמצא למרות ה-✓:** ברוסית אותו בוט קרא לעצמו ״Я — Barak״,
״Меня зовут Barak״ ו-״Барак״. הרשימה ב-`CORE` הייתה שטוחה, ולכן
המודל ניחש איזו צורה שייכת לאיזו שפה — חד־משמעי בכתב הערבי, לא
ברוסית. היום כל צורה קשורה לקוד השפה שהבקשה נושאת.

**הכלל שיצא מזה, והוא לסשן הבא:** ✓ בהערכות אינו ״תקין״ אלא ״לא
נפל על מה שנבדק״. פעמיים באותו ערב הפסק היה ירוק והפלט הראה תקלה —
ארבעה ✓ שהסתירו אותיות עבריות, ושני ✓ שהסתירו שתי צורות של אותו
שם. **קוראים את מה שהמודל כתב, לא רק את השורה האחרונה.**
