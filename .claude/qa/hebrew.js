/* =====================================================================
   העברית של לימור — `node .claude/qa/hebrew.js`

   **מקור: דיווח הבעלים, 18.9.2026 — ״העברית של לימור לא תקינה״.**
   ...
   **מה שהמדידה גילתה, וזה העיקר: ללימור שלושה מקורות עברית, ורק
   אחד מהם היה מכוסה.**
   ...
   **2. ההוראה והשומר אינם נפרדים.** `LANGRULE.he` נוקב בתבניות
   ...
   ולימור ימשיך לכתוב אותה בזמן שהפרומפט אוסר. הבדיקה דורשת שכל
...
/* שלושת קובצי הלקוח — כל מה שלימור אומר בלי לעבור בשרת.
...
/* **מילות מפתח שהלומד מקליד אינן מה שלימור אומר.** `josh-local.js`
...
(async () => {
  ...
  console.log('\n' + seen + ' מחרוזות עבריות של לימור נבדקו, ' + bad + ' ממצאים');
