/* =====================================================================
   ...
   2. **המבנה.** כל טקסט הוא אובייקט {he} — מוכן לארבע שפות —
      ולא מחרוזת חשופה.
   3. **האורך.** 260–620 תווים, 4–7 משפטים (הורחב 18.9.2026 בהוראת
      הבעלים: ״קצת יותר תוכן… יותר מעניין לקרוא״ — כל טקסט נושא
      דוגמה יומיומית). קורא עם דיסלקציה מסיים פסקה של שישה משפטים
      קצרים; קטע קצר מדי אינו אומר דבר.
   3ב. **דוגמה.** כל טקסט מכיל ״למשל״ — הדוגמה היא מה שהופך נטייה
      למשהו שאפשר לזהות בחיים.
...
for (const [key, he] of items) {
  ...
  if (len < 260 || len > 620) { bad++; console.log(`✗ ${key}: ${len} תווים (260–620)`); }
  if (sent < 4 || sent > 7) { bad++; console.log(`✗ ${key}: ${sent} משפטים (4–7)`); }
  if (he.indexOf('למשל') < 0) { bad++; console.log(`✗ ${key}: אין ״למשל״ — חסרה דוגמה`); }
