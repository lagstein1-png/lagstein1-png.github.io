...
/* נרמול: מה שמותר להשתנות בין אפליקציה לאפליקציה.
   ...
   **ונכס תמונה מקומי אינו מנוע.** `PRE` מונה גם קבצים שהם רכוש
   האפליקציה ולא קוד משותף — האייקונים, ומ-11.9.2026 גם
   `josh-sprites.png` ב-math-app. **הורחב 12.9.2026 גם ל-jpg/webp/svg**
   כשנכנס `./img/josh.jpg` — הדיוקן של ג׳וש, נכס של אפליקציה אחת
   בדיוק כמו האייקונים. הכלל לא השתנה, רק סיומות הקובץ.
   ההבדל הזה הפיל את הבדיקה על
   math-app בשלוש ריצות רצופות ב-`main`, והוא אינו סחיפה אלא בדיוק
   מה שאמור להיות שונה: תמונה של אפליקציה אחת. הנתיבים המשותפים
   (`/tutor/`, `/legal/`) אינם נכנסים לנרמול הזה ונשארים תחת שמירה,
   וקיום הנתיבים שב-`PRE` נבדק ממילא ב-`cache.js`. */
const normCache   = s => s.replace(/"[a-z0-9-]+-"\s*\+\s*V/g, '"APP-"+V')
                          .replace(/startsWith\("[a-z0-9-]+-"\)/g, 'startsWith("APP-")')
                          .replace(/\s*"\.\/[^"]*\.(?:png|jpe?g|webp|svg)",?/g, '')
                          /* שורת הטייס של שלב 2. `math-app` היא האפליקציה
                             היחידה שמחווטת לשכבת הפנים, ולכן היא לבדה מצרפת
                             מראש את `/tutor/josh-face.js` — וזו סחיפה אמיתית
                             לזמן קצוב, לא סחיפה שצריך להסתיר.

                             **מנורמלת בשמה בלבד**, ולא ככלל: הכלל שכתוב שלוש
                             שורות מעל אומר שנתיבי `/tutor/` ו-`/legal/`
                             נשארים תחת שמירה, ושורה אחת ידועה אינה מבטלת
                             אותו. קיום הנתיב נבדק ממילא ב-`cache.js` בדיקה 4.

                             **מתי זה נמחק:** בשלב 6, כשכל האפליקציות יקבלו
                             את השורה — אז הנרמול הופך ל-no-op, ומחיקתו לא
                             תשנה דבר. מי שמגיע לכאן ורואה שהשורה קיימת בכל
                             הקבצים — ימחק את שלוש השורות האלה. */
                          .replace(/\s*"\/tutor\/josh-face\.js",?/g, '');
