...
const normCache   = s => s.replace(/"[a-z0-9-]+-"\s*\+\s*V/g, '"APP-"+V')
                          ...
                          /* גלאי מצב הלומד — **תשע מתוך אחת־עשרה**.
                             13.9.2026 הוא הופץ מ-`math-app` לשמונה
                             נוספות, ושתיים נשארו בחוץ **בכוונה**:
                             `reader` ו-`kotvim` אין בהן נכון/לא־נכון
                             כלל, ולכן אין לגלאי מה למדוד שם.

                             זו אינה סחיפה אלא הבדל מוצדק, ולכן הוא
                             מנורמל ולא מדווח. **בשמו בלבד** ולא ככלל:
                             הכלל שלמעלה אומר שנתיבי `/tutor/` ו-`/legal/`
                             נשארים תחת שמירה, ושורה אחת ידועה אינה
                             מבטלת אותו. קיום הנתיב נבדק ממילא
                             ב-`cache.js` בדיקה 4, והחיווט עצמו
                             ב-`joshstate.js`.
