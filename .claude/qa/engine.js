...
const normCache   = s => s.replace(/"[a-z0-9-]+-"\s*\+\s*V/g, '"APP-"+V')
                          ...
                          /* שורת הטייס של שלב 3, ובדיוק כמו זו של שלב 2
                             לפניה (`O-53`): `math-app` היא האפליקציה
                             היחידה שמחווטת לגלאי מצב הלומד, ולכן היא
                             לבדה מצרפת מראש את `/tutor/josh-state.js`.
                             זו סחיפה אמיתית לזמן קצוב, ולא סחיפה שצריך
                             להסתיר.

                             **מנורמלת בשמה בלבד**, ולא ככלל: הכלל שכתוב
                             למעלה אומר שנתיבי `/tutor/` ו-`/legal/`
                             נשארים תחת שמירה, ושורה אחת ידועה אינה
                             מבטלת אותו. קיום הנתיב נבדק ממילא
                             ב-`cache.js` בדיקה 4.

                             **מתי זה נמחק:** כשהגלאי יופץ לשאר
                             האפליקציות — אז הנרמול הופך ל-no-op,
                             ומחיקתו לא תשנה דבר. זה קרה בדיוק כך
                             לשורה של שלב 2, שנמחקה 13.9.2026. */
