...
(async () => {
  ...
  /* **״עוד 2 משפטים״ → ״עוד שני משפטים״, 18.9.2026.**
     הבדיקה הזאת נעשתה אדומה על שיפור אמיתי, וזה בדיוק תפקידה:
     `more` היה מחרוזת אחת עם `{n}`, ולכן שארית של אחד נתנה
     **״ויש עוד 1 משפטים״**. הוא פוצל ל-`one`/`two`/`many`
     (ולרוסית גם `few`, לפי `n % 10` ו-`n % 100`).

     שלוש טענות במקום אחת — השארית 1 היא הבאג שתוקן, והיא
     חייבת להיבדק ולא רק הרבים. */
  t('שארית שתיים — זוגי מנייני', !!s && /עוד שני משפטים/.test(s.text), true);
  const s1 = win.JOSHLOCAL.simplify(
    'אחת. שתיים. שלוש. ארבע. חמש. שש. שבע. שמונה. תשע.', 'he');
  t('שארית אחת — יחיד, ולא ״1 משפטים״',
    !!s1 && /עוד משפט אחד/.test(s1.text) && !/1 משפטים/.test(s1.text), true);
  const sr = win.JOSHLOCAL.simplify(
