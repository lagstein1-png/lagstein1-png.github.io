...
import(WORKER).then(async W => {
  ...
  if (!fs.existsSync(PATCH)) { bad++; console.log('✗ tutor-api/theory.patch אינו קיים') }
    ...
    /* **עותק מקומי נחשב** — מנוע לימור, D-10 (16.9.2026): הריפו הנפרד
  ...
  t('הגוף המשותף נושא את השם לימור', /שמך לימור/.test(W.CORE), true);
  ...
  (function () {
    ...
    /* קול נשי נבחר אוטומטית מבין קולות זמינים. */
    ctx.LIST = [V('Carmit'), V('Google עברית'), V('Microsoft Asaf')];
    t('בוחר קול נשי מבין קולות מקומיים',
    ...
    t('קול נשי נשאר עדיף כשהרשת חוזרת',
      (ctx.out.pick('he-IL') || {}).name, 'Carmit');
    ...
    t('הקול שנבחר ביד גובר על המיון',
      (ctx.out.pick('he-IL') || {}).name, 'Carmit');
    ctx.out.save('he-IL', '');
    t('ביטול הבחירה מחזיר בחירה נשית אוטומטית',
      (ctx.out.pick('he-IL') || {}).name, 'Carmit');

    ctx.LIST = [V('Microsoft Asaf')];
    t('קול יחיד שאינו נשי עדיין נשמע', (ctx.out.pick('he-IL') || {}).name, 'Microsoft Asaf');
    ctx.LIST = [V('Carmit'), V('Google עברית'), V('Microsoft Asaf')];
  ...
  /* **מנוע לימור, 16.9.2026 — המונה עבר לזיכרון עם flush ל-KV.**
