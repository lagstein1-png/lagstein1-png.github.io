...
import(WORKER).then(async W => {
  ...
  (function () {
    ...
    /* **ההרמה כשאין קול נשי חייבת להיות זו של האפליקציות.**
       נמדד 13.9.2026: ג׳וש קיבל 1.12 — שהוא ה-pitch של הפריסט
       `calm-female`, כלומר הגובה של קול נשי אמיתי — במקום 1.35,
       שהיא ההרמה כשאין כזה. הבעלים שמע קול גברי, וההפרש הזה הוא
       בדיוק הסיבה. הבדיקה משווה לאפליקציות ולא למספר קשיח, ולכן
       שינוי שם יגרור גם את ג׳וש. */
    /* **מה שמשווים הוא מה שנשמע, ולא הקבוע.** האפליקציות מכפילות
       את FEM_PITCH בגובה הפריסט וחוסמות בתקרה, ולכן על מכשיר בלי
       קול נשי הן משמיעות את התקרה. לג׳וש אין פריסט, ולכן הערך
       שלו חייב להיות התקרה עצמה — אחרת הוא נמוך מהאפליקציה
       שהוא יושב בתוכה. */
    const appCeil = (() => {
      const src = fs.readFileSync(path.join(ROOT, 'math-app', 'index.html'), 'utf8');
      const m = src.match(/Math\.min\(([\d.]+),\s*pitch\s*\*\s*femLift/);
      return m ? +m[1] : null;
    })();
    const botPitch = (() => { const m = client.match(/var FEM_PITCH = ([\d.]+)/); return m ? +m[1] : null })();
    t('גובה הנפילה של ג׳וש = התקרה שהאפליקציות משמיעות', botPitch, appCeil);
