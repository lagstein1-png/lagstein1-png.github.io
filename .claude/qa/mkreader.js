...
/* =====================================================================
   `--check` — הרינדור מול מה שנמצא בדיסק, בלי לכתוב.

   **למה זה נדרש: הקובץ נסחף בשקט, ואיש לא ראה.** נמדד 16.9.2026.
   `marketing/index.html` נשא את הודעת התקשורת **הישנה** — ״שנבנו
   במיוחד עבור תלמידים עם דיסלקציה״ — בזמן ש-`campaign.md` כבר
   תוקן. ניווט הקובץ אף הכריז ״facts.md · 9.3 KB״ בזמן שהקובץ
   היה 10.1 KB, כלומר הפער היה **כתוב בדף עצמו** ואיש לא הסתכל.

   המחולל היה קיים כל הזמן. מה שלא היה קיים הוא מי שיבדוק
   שהריצו אותו — עורכים `.md`, שוכחים את הפקודה, והדף המשותף
   מגיש טקסט שנפסל. הבדיקה הזאת היא בדיוק מה שחסר.

   **וטענה שיווקית שגויה אינה מתגלה כמו באג** — אין מסך שנשבר,
   יש הורה שהאמין למשפט. `claims.js` סורק את שני הקבצים, ולכן
   טענה שנמחקה מה-`.md` וחיה ב-HTML הייתה נופלת שם ממילא —
   אבל רק אם מישהו כבר הריץ. פער שאינו טענה אסורה (מספר מיושן,
   פסקה שנמחקה) לא נתפס שם כלל.
   ===================================================================== */
const OUT = path.join(DIR, 'index.html');
if (process.argv.includes('--check')) {
  const cur = fs.existsSync(OUT) ? fs.readFileSync(OUT, 'utf8') : '';
  if (cur === HTML) {
    console.log('✓ ' + OUT + ' מסונכרן עם ' + files.length + ' קובצי המקור');
    process.exit(0);
  }
  console.log('✗ ' + OUT + ' אינו מה שקובצי ה-.md מייצרים היום');
  console.log('  קובץ .md נערך והמחולל לא רץ. התיקון: node .claude/qa/mkreader.js');
  process.exit(1);
}
