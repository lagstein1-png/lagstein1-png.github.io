/* =====================================================================
   barak.js — מנוע לימור: חוזה ה-Worker מול מודל מדומה, והחיווט בלקוח
...
(async () => {
  ...
  /* 12ב. **איכות השפה — נוסף 17.9.2026.** הבעלים: ״העברית שלו
     ...
     **שני צדדים, ובמתכוון בשני מקומות שונים.** הכלל הכללי יציב
     ...
     — לומד שקורא עברית שגויה מלימור לומד אותה. */
  t('CORE: שפה תקנית', /שפה תקנית ונכונה/.test(W.CORE), true);
  t('CORE: אין העתקת תחביר מאנגלית', /אל תעתיקי מבנה תחבירי משפה אחרת/.test(W.CORE), true);
  ...
  /* **השם בארבעה כתבים — 16.9.2026, מריצת barak-live 3.**
     ...
     ״أنا לימור״ בערבית ו-״Я לימור״ ברוסית — אותיות עבריות באמצע משפט
     שלומד ערבי או רוסי קורא — וגם بارق מול ليمور, Барк מול Лимор,
     ו-barak באות קטנה. שלוש מהן **עברו** את ההערכות, מפני שהסף שם
     היה ארבעה תווים עבריים רצופים ו״לימור״ הוא שלושה.
  ...
  const NAMES = { he: 'לימור', ar: 'ليمور', ru: 'Лимор', en: 'Limor' };
  ...
  console.log(bad ? `✗ barak.js — ${bad} ממצאים` : '✓ מנוע לימור — החוזה, הפעולות והחיווט');
