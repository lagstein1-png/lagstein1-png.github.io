...
הריצה של `main` היא הדבר היחיד שרואה את הקוד אחרי שהוא נדחף, ואיש
אינו מסתכל בה: בריפו של ״תאוריה מדברת״ החבילה הייתה אדומה בשלוש
גרסאות רצופות — v96, v97 ו-v98, ריצות 55, 56 ו-57 לפי הודעת הקומיט
`2e2055c` שם — עד שסשן נתקל בה במקרה. אדום שיושב על `main` הופך את הריצה הבאה לחסרת משמעות —
כשהיא אדומה גם לך, אין לדעת אם זה אתה.

בודקים לפני שנוגעים בכלום: GitHub → Actions → הריצה האחרונה על
`main`. אדומה — מתקנים **אותה** קודם, בקומיט נפרד, ורק כשהיא ירוקה
ממשיכים לשלב 1. ריפו בלי workflow — אין מה לבדוק, וכותבים זאת
בדיווח במקום ״ירוק״.
...
הרשימה שהמשתמש אישר כאן היא הרשימה שתיכנס ל-`git add` בשלב 6.
...
## שלב 3 — הגרסה באמת עלתה, ולא רק תואמת

`cache.js` כבר רץ בשלב 2 והוכיח ש-`BUILD` תואם ל-`?v=` בכל
...
## שלב 4 — שתי הבדיקות האיטיות, על מה שנגעו בו

שלב 2 רץ ב-`--fast` ודילג על `entropy` ועל `options`. הן אלה
...
```
git add <רק הקבצים שאושרו בשלב 1, מפורשים בשם>
git commit -m "<תיאור>"
git tag good-v58
```

מספר התגייה הוא **אותו מספר** של `BUILD` משלב 3. לא חדש, לא +1.
