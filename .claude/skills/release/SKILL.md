...
**שער הסיום כבר הריץ את החלק הסטטי.** `.claude/hooks/qa-gate.sh`
מריץ `--static` בכל ניסיון לסגור סשן, ולכן שש־עשרה בדיקות כבר
ירוקות כשמגיעים לכאן. `--fast` מוסיף עליהן את `smoke` ואת `exam`,
שדורשים דפדפן ולכן אינם בשער.
