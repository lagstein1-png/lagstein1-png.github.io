/* =====================================================================
   שיא — בגרות בהיסטוריה (22261/22262) — לוגיקה.
   נבנה על שיא 5 יח״ל (bagrut-806, BUILD x103).
...
(function () {
  ...
  /* מקורות היסטוריים ארוכים נשברים לפסקאות לפי שורות, וכל פסקה
     עוברת sentHtml כדי שההקראה תדגיש משפט־משפט כרגיל. */
  function paraHtml(text) {
    return String(text == null ? "" : text).split(/\n+/).filter(function (p) {
      return p.trim();
    }).map(function (p) { return '<p class="srcp">' + sentHtml(p) + "</p>"; }).join("");
  }
  function qImg(q) {
    return (q.images || []).map(function (im) {
  ...
  /* שני השאלונים מוצגים בקבוצות נפרדות: 22261 (70% מהציון) ו-22262
     (30%). תלמיד שלומד לבגרות צריך לראות מיד לאיזה שאלון כל
     הדגמה שייכת — הציון הסופי הוא שקלול של השניים. */
  var QINFO = {
    "22261": { name: "שאלון 22261", sub: "שאלון רשמי של משרד החינוך · 70% מהציון · שלוש שעות · ארבעה פרקים, בחירה בשלושה מהם · התשובות הסופיות יתווספו עם מפתח ההערכה הרשמי",
               link: "https://meyda.education.gov.il/sheeloney_bagrut/2025/1/HEB/22261.pdf" },
    "22262": { name: "שאלון 22262", sub: "שאלון רשמי של משרד החינוך · 30% מהציון · שעה וחצי · בית שני — מעם מקדש לעם הספר · התשובות הסופיות יתווספו עם מפתח ההערכה הרשמי",
               link: "https://meyda.education.gov.il/sheeloney_bagrut/2025/1/HEB/22262.pdf" }
  };
  function renderHome() {
    var all = window.EXAMS || [];
    var box = $("#exam-list");
  ...
  function renderSim() {
    ...
    if (!SIM.on) {
      ...
      box.innerHTML = '<p class="note">בסימולציה אין רמזים ואין פתרונות עד הסיום, ' +
        ...
        (ex.source ? '<p class="meta"><a href="' + ex.source.url + '" target="_blank" rel="noopener">' + esc(ex.source.label) + "</a> · שאלון רשמי של משרד החינוך</p>" : "") +
        chooseHtml +
  ...
  function go(screen) {
    ...
    document.title = TITLES[screen] +
      (screen === "home" ? " — בגרות בהיסטוריה, שאלונים 22261 ו-22262" : " · שיא — בגרות בהיסטוריה");
  }
      ...
      BARAK.register({
        ...
        getScreenContext: function () {
          ...
          return {
            ...
            level: "היסטוריה לבגרות",
            curriculum: "בגרות בהיסטוריה (22261/22262) — " + (it.q.topic || "")
          };
        },
        actions: {
  ...
  if ("serviceWorker" in navigator && location.protocol !== "file:") {
    window.addEventListener("load", function () {
      navigator.serviceWorker.register("sw.js?v=h103-pwa1").catch(function () {});
    });
  }
