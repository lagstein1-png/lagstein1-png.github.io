...
## המפתח — אל תערוך ביד
    ...
    node tutor-api/local/setkey.js

מדביקים את המפתח (הוא אינו מוצג), והכלי כותב את `.env` בעצמו,
מוחק מופעים קודמים, ומאמת מהדיסק שמה שנכתב הוא מה שהודבק.
העריכה הידנית נכשלת בשקט בחמש דרכים — קובץ בתיקייה אחרת, עורך
שלא שמר, מפתח בתוך שורת הערה, מרכאות או רווח, ושורה שנייה
שפרסר אחר קורא במקום הראשונה.

## לא עולה? הרץ אבחון

    node tutor-api/local/doctor.js

הוא בודק שבעה דברים — גרסת Node, שהקבצים במקומם, שהייבוא באמת
עובד, שהפורט פנוי, שהמפתח קיים ונראה נכון, ושה-`.env` חסום
ב-`.gitignore` — ואומר לכל כשל מה לעשות. **אינו פונה לרשת ואינו
עולה כסף.**
